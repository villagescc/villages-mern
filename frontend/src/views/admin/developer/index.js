            ...
            >
                <TableContainer>
                    <Table sx={{ minWidth: 350 }} aria-label="simple table">
                        ...
                        <TableBody>
                            {!loading ? developerApproval?.map((row, index) => (
                                <TableRow hover key={index}>
                                    <TableCell>
                                        <Typography variant="body1" component={'span'}>{row.applicationName}</Typography>
                                    </TableCell>
                                    <TableCell>
                                        <Typography variant="body1" component={'span'}>{row.user.username}</Typography>
                                    </TableCell>
                                    <TableCell>
                                        <Typography variant="body1" component={'span'}>{row.clientSecret}</Typography>
                                    </TableCell>
                                    <TableCell>
                                        <Link href={`${row.redirectUrl}`} target="_blank" >{row.redirectUrl}</Link>
                                    </TableCell>
                                    <TableCell>{moment(row.createdAt).format('YYYY/MM/DD HH:mm:ss')}</TableCell>
                                    <TableCell>
                                        <Stack direction="row" justifyContent="center" alignItems="center" spacing={2}>
                                            <AnimateButton>
                                                <Button type="submit" variant="contained" color="success" disabled={row.isApproved} onClick={() => isApproveDeveloper(row._id)}>
                                                    Approve
                                                </Button>
                                            </AnimateButton>
                                        </Stack>
